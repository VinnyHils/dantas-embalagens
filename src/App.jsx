...
function App() {
  ...
  const visibilityTrackedRef = React.useRef(false);
  ...
  // Controla exibição do botão flutuante após rolar a página
  useEffect(() => {
    const onScroll = () => {
      const shouldShow = window.scrollY > 300;
      setShowWhatsApp(shouldShow);
      if (shouldShow && !visibilityTrackedRef.current) {
        track('cta_visible', { cta: 'whatsapp_floating' });
        visibilityTrackedRef.current = true;
      }
    };
    window.addEventListener('scroll', onScroll, { passive: true });
    // Chamada inicial (caso usuário entre em rota já rolada por navegação com hash no futuro)
    onScroll();
  ...
  <Header activeSection={activeSection} />
      ...
      <a
        ...
        className={`fixed bottom-5 right-5 z-50 group transition-all duration-300 ${showWhatsApp ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-4 pointer-events-none'}`}
        onClick={() => track('cta_click', { cta: 'whatsapp_floating' })}
      >
        <div className="relative flex items-center justify-center w-14 h-14 rounded-full bg-gradient-to-r from-green-500 to-green-600 shadow-lg shadow-green-600/30 hover:shadow-green-600/50 hover:scale-105 active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500/60 transition-all">
          ...
          <span className="absolute -top-2 -right-2 bg-white text-green-600 text-[10px] font-bold px-2 py-0.5 rounded-full shadow border border-green-500/30">WhatsApp</span>
        </div>
      </a>
    </div>
  );
